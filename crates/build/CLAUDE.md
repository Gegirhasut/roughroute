...
## Structure (DECISIONS D8 — keep the layering)
- **Pure layer** (the bulk): in-memory road network (ways as node-id sequences
  + highway class, node id → coordinate map) → `core::Graph`. Two stages:
